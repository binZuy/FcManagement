  ...
  try {
    ...
    if (!title || !matchDate) {
      return NextResponse.json(
        { error: "Tên trận đấu và thời gian là bắt buộc" },
        { status: 400 }
      );
    }
    ...
    // Check uniqueness with while loop to prevent race/counter collision
    ...
    const match = await prisma.matchSession.create({
      data: {
        ...
        opponentName: matchType === MatchType.INTERNAL ? null : opponentName,
    ...
    return NextResponse.json(
      { error: err?.message || "Lỗi tạo trận đấu" },
      { status: 500 }
    );
  }
}
